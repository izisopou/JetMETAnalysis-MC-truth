...
cp ${CMSSW_BASE}/src/JetMETAnalysisMCtruth/JetAnalyzers/scripts/delphes.xml ${CMSSW_BASE}/config/toolbox/${SCRAM_ARCH}/tools/selected
scram setup delphes
...
# Check if the Delphes folder/library exist and if the library is commented out of the JetMETAnalysisMCtruth/JetAnalyzers/bin/BuildFile.xml. If so then uncomment the library line. Do the same for the DelphesNtupleToJRANtuple_x.cc executable.
export BUILD_FILE_PATH=${CMSSW_BASE}/src/JetMETAnalysisMCtruth/JetAnalyzers/bin/BuildFile.xml
if [ -d "$DELPHES_DIR" ]; then
	...
	then
		...
		echo "The file libDelphes.so does not exist where we expected. Cannot include library in the JetMETAnalysisMCtruth/JetAnalyzers/bin/BuildFile.xml not can we compile DelphesNtupleToJRANtuple_x.cc."
		exit 1
	fi
fi
