...
#include "JetMETAnalysisMCtruth/JetAnalyzers/interface/JetResponseAnalyzer.hh"
...
JetResponseAnalyzer::JetResponseAnalyzer(const edm::ParameterSet& iConfig)
  ...
  , srcgenVtx_              (consumes<ROOT::Math::PositionVector3D<ROOT::Math::Cartesian3D<float>,ROOT::Math::DefaultCoordinateSystemTag> >(iConfig.getParameter<edm::InputTag>            ("srcgenVtx")))
                                  ...
                                  const edm::EventSetup& iSetup)
{
  ...
  edm::Handle<ROOT::Math::PositionVector3D<ROOT::Math::Cartesian3D<float>,ROOT::Math::DefaultCoordinateSystemTag> >    genvtx;
  ...
  if (iEvent.getByToken(srcVtx_,vtx)) {
     ...
     for (reco::VertexCollection::const_iterator vtxIter = vtx->begin(); vtxEnd != vtxIter; ++vtxIter) {
        if (!vtxIter->isFake() && vtxIter->ndof()>=4 && fabs(vtxIter->z())<=24) {
           ++(JRAEvt_->npv);
	   JRAEvt_->recopvz->push_back(vtxIter->z());
	   //cout << "Reco vertex z = " << vtxIter->z() << endl;
        }
     }
  }

 
  //GEN PV Z INFORMATION
  JRAEvt_->genpvz = -1000;
