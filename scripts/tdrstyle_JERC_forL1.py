...
def setTDRStyle():
  ...
  tdrStyle.SetTitleSize(0.06, "XZ")
  tdrStyle.SetTitleSize(0.045,"Y") #0.045
...
def tdrCanvas(canvName, x_min, x_max, y_min, y_max, nameXaxis, nameYaxis, square=kRectangular, iPos=11, is2D=False, isExtraSpace=False):
  ...
  L = 0.15*H_ref if square else 0.16*H_ref  #different between cpp and python
  R = 0.05*H_ref if square else 0.05*H_ref  #different between cpp and python
  ...
  h.GetYaxis().SetTitleOffset((0.8 if is2D else 1.8) if square else (1.3 if isExtraSpace else 1.0)) #1.8
