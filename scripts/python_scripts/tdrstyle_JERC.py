...
commonScheme = {
  ...
  'legend': {
    ...
    'Run2':       'Run 2',
    'Run3':       'Run3',
    'Summer22_V1':     'Summer22',
    'Summer22EE_V1':   'Summer22EE',
    'Summer23_V1':     'Summer23',
    'Summer23BPix_V1': 'Summer23BPix',
    },
  'lumi': {
    'UL16APV':    '20',
    'UL16nonAPV': '17',
...
def SetLumi(year, isSimulation=False):
  global cms_lumi
  cms_lumi = commonScheme['legend'][year]
  if 'UL' in year: cms_lumi += ' Legacy'
  if not isSimulation:
    cms_lumi += ' , '+commonScheme['lumi'][year]+' fb^{-1}'
...
def tdrCanvas(canvName, x_min, x_max, y_min, y_max, nameXaxis, nameYaxis, square=kRectangular, iPos=11, is2D=False, isExtraSpace=False):
  ...
  if not square and isExtraSpace and is2D:
    canv.SetRightMargin( B/W+0.1)
  canv.SetTopMargin(T/H)
  canv.SetBottomMargin(B/H + 0.02)

  # Draw frame and set axis labels
  h = canv.DrawFrame(x_min, y_min, x_max, y_max)
  y_offset = (1.15 if isExtraSpace else 1.2) if square else (1.0 if isExtraSpace else 0.8)  #1.15 for offset
